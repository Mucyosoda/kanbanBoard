...
const popUp = pokemon => {
  ...
  fetch(pokemon.location_area_encounters).then(response => response.json()).then(locations => {
  ...
  submitBtn.addEventListener('click', () => {
    if (nameInput.value.length < 1 || nameInput.value.length > 8) {
      ...
      postComments(nameInput.value, textArea.value, pokemon.name).then(() => getComments(pokemon.name).then(() => showComments()));
      nameInput.classList.remove('red');
      textArea.classList.remove('red');
      addComment.reset();
    }
